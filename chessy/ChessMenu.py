...
class Card:
    ...
    def draw(self, surface, scroll_offset):
        ...
        # Kartın arka planını çiz
        if self.is_flipped:
            p.draw.rect(surface, (200, 100, 255), self.rect, border_radius=15)  # Arka taraf rengi
            draw_text_in_box(surface, "", self.lines, self.rect, self.big_font, self.small_font)
        else:
            p.draw.rect(surface, (123, 6, 158), self.rect, border_radius=15)  # Ön taraf rengi

            # Başlığı manuel olarak satır satır böl
            wrapped_title = self.title.split(' ')
    ...
    def handle_event(self, event, mouse_pos):
        if event.type == p.MOUSEBUTTONDOWN and event.button == 1:
            if self.rect.collidepoint(mouse_pos):
                self.is_flipped = not self.is_flipped  # Kartı çevir
